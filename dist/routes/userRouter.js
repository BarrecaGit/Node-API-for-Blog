...
userRouter.get('/Login/:us/:pwd', (req, res, next) => {
    ...
    if (person) {
        bcrypt_1.default.compare(req.params.pwd, person?.password, (err, result) => {
            if (result) {
...
userRouter.post('/', (req, res, next) => {
    ...
    var validEmail = checkEmail(newUser.emailAddress);
    if (!validEmail) {
